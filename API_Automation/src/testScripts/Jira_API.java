...
public class Jira_API extends BaseClass {
	 ...
	 */
	@Test(dependsOnMethods = "getBugOrissue")
	public static void deleteAttachment() {
		
		// The JIRA URL of the project
		screenshotID = screenshotID.replaceAll("\\[", "");
		screenshotID = screenshotID.replaceAll("\\]", "");
		RestAssured.baseURI = "https://aaaryanstar.atlassian.net/";
		// Creating a request with specification
	...
	/*
	 * This will run a test to Get the issue.
	 */
	@Test(dependsOnMethods="addAttachment")
	public static void getBugOrissue() {
		
		// The JIRA URL of the project
		RestAssured.baseURI = "https://aaaryanstar.atlassian.net/";
		// Creating a request with specification
		RequestSpecification request = given().log().all().header("Accept","application/json").header("Authorization", token);
		String response = getIssue(request, issueID);
