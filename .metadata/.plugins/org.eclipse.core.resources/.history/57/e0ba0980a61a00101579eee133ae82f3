...
public class DynamicJson extends BaseClass {
	...
	public static void addBook(String isbn, String asile) throws IOException {
...
//		Assert.assertEquals(respMsg, "book is successfully deleted");
	...
	/*
	 * This test will add book from the JSON file stored
	 */
	@Test
	public static void addBookUsingJsonFile() throws IOException {
		RestAssured.baseURI = "http://216.10.245.166";
		RequestSpecification req = given().log().all().header("Content-Type", "application/json");
		String addBookRes = "/Library/Addbook.php";

		String response = performPost(req, addBookRes,
				new String(Files.readAllBytes(Paths.get(".\\src\\files\\addBook.json"))));

		String reqRes = fetchValueByParsingStringToJson(response, "ID");

		System.out.println(reqRes);

		Assert.assertEquals("This is Test1", reqRes);
	}
